...
class ParityNetworkAnalyzer:
    def __init__(self, d=30, k=6, M1=128, M2=128, learning_rate=0.01, 
        ...
        # NEW: Track ρⱼ(t) for selected neurons over time
        self.rho_j_history = {
            'epochs': [],
            'neurons': {},  # Will be populated with sampled neurons
            'mean': []      # Average correlation across sampled neurons
        }
        
        # NEW: Track W2 positivity ratio
        self.w2_positivity = {
            'epochs': [],
            'positive_ratio': []
        }
        ...
        # NEW: Select 10 random neurons to track ρⱼ(t)
        self.tracked_neurons = np.random.choice(self.M1, size=10, replace=False)
        for neuron_idx in self.tracked_neurons:
            self.rho_j_history['neurons'][neuron_idx] = []
    ...
    def compute_neuron_correlations(self, epoch):
        ...
        with torch.no_grad():
            ...
            for j in range(self.M1):
                ...
                # NEW: Record ρⱼ(t) for tracked neurons
                if j in self.tracked_neurons:
                    self.rho_j_history['neurons'][j].append(corr)
            
            # NEW: Record average correlation
            self.rho_j_history['epochs'].append(epoch)
            self.rho_j_history['mean'].append(np.mean([abs(c) for c in layer1_corrs]))
    ...
    def compute_feature_importance(self, epoch):
        ...
        # NEW: Compute W2 positivity ratio
        W2 = self.model.W2.detach().cpu().numpy()
        positive_ratio = np.mean(W2 > 0)
        
        # Store W2 positivity ratio
        self.w2_positivity['epochs'].append(epoch)
        self.w2_positivity['positive_ratio'].append(positive_ratio)
    ...
    def visualize_theory_validation(self):
        """
        Create plots to validate theoretical predictions about:
        1. Network-target correlation C(t)
        2. Neuron-target correlations ρⱼ(t)
        3. Weight matrix evolution (feature importance ratio and W2 positivity)
        """
        plt.figure(figsize=(15, 10))
        
        # Extract correlation history
        epochs, correlations = zip(*self.correlation_history)
        
        # Create four panel figure with shared x-axis
        gs = GridSpec(4, 1, figure=plt.gcf(), hspace=0.3)
        
        # 1. Network correlation C(t)
        ax1 = plt.subplot(gs[0])
        ax1.plot(epochs, correlations, 'b-', marker='o', label='C(t)')
        ax1.set_ylabel('Network Correlation C(t)')
        ax1.set_title('Network-Target Correlation C(t)')
        ax1.grid(True)
        
        # 2. Average absolute neuron correlation ρⱼ(t)
        ax2 = plt.subplot(gs[1], sharex=ax1)
        ax2.plot(self.rho_j_history['epochs'], self.rho_j_history['mean'], 'g-', marker='o', 
                label='Mean |ρⱼ(t)|')
        
        # Plot individual tracked neurons
        for neuron_idx, corrs in self.rho_j_history['neurons'].items():
            if len(corrs) > 0 and max([abs(c) for c in corrs]) > 0.2:  # Only plot neurons with significant correlation
                ax2.plot(self.rho_j_history['epochs'], corrs, '--', alpha=0.5, 
                        label=f'Neuron {neuron_idx}')
        
        ax2.set_ylabel('Neuron Correlation |ρⱼ(t)|')
        ax2.set_title('Neuron-Target Correlations ρⱼ(t)')
        ax2.grid(True)
        ax2.legend(loc='upper left')
        
        # 3. Feature importance ratio (relevant vs. irrelevant)
        ax3 = plt.subplot(gs[2], sharex=ax1)
        ax3.plot(self.feature_importance['epochs'], self.feature_importance['ratio'], 
                'r-', marker='o', label='W1 Relevant/Irrelevant Ratio')
        ax3.set_ylabel('Feature Importance Ratio')
        ax3.set_yscale('log')
        ax3.set_title('Relevant/Irrelevant Feature Importance Ratio (log scale)')
        ax3.grid(True)
        
        # 4. W2 positivity ratio
        ax4 = plt.subplot(gs[3], sharex=ax1)
        ax4.plot(self.w2_positivity['epochs'], self.w2_positivity['positive_ratio'], 
                'c-', marker='o', label='W2 Positive Ratio')
        # Add a horizontal line at 0.5 (random initialization expectation)
        ax4.axhline(y=0.5, color='gray', linestyle='--', alpha=0.7, label='Random Init (50%)')
        ax4.set_ylabel('Proportion of W2 > 0')
        ax4.set_title('W2 Positivity Ratio (Testing the "Voting Mechanism")')
        ax4.grid(True)
        ax4.legend(loc='upper left')
        ax4.set_xlabel('Epoch')
        
        # Add vertical lines for correlation thresholds
        correlation_thresholds = [0.5, 0.9, 0.95]
        for threshold in correlation_thresholds:
            for i, (epoch, corr) in enumerate(self.correlation_history):
                if corr > threshold:
                    for ax in [ax1, ax2, ax3, ax4]:
                        ax.axvline(x=epoch, color=f'C{correlation_thresholds.index(threshold)+3}', 
                                linestyle='--', alpha=0.7,
                                label=f'Corr > {threshold}' if ax == ax1 else "")
                    break
        
        ax1.legend(loc='lower right')
        
        plt.tight_layout()
        plt.savefig(f"{self.save_dir}/theory_validation_d{self.d}_k{self.k}.png", dpi=300)
        plt.close()
        
        # Create additional combined plot of feature importance ratio and W2 positivity
        plt.figure(figsize=(12, 8))
        
        # Primary y-axis: Feature importance ratio (log scale)
        ax1 = plt.gca()
        ax1.plot(self.feature_importance['epochs'], self.feature_importance['ratio'], 
                'r-', marker='o', linewidth=2, label='W1 Relevant/Irrelevant Ratio')
        ax1.set_xlabel('Epoch')
        ax1.set_ylabel('Feature Importance Ratio (log scale)', color='r')
        ax1.tick_params(axis='y', labelcolor='r')
        ax1.set_yscale('log')
        
        # Secondary y-axis: W2 positivity ratio
        ax2 = ax1.twinx()
        ax2.plot(self.w2_positivity['epochs'], self.w2_positivity['positive_ratio'], 
                'b-', marker='x', linewidth=2, label='W2 Positive Ratio')
        ax2.set_ylabel('Proportion of W2 > 0', color='b')
        ax2.tick_params(axis='y', labelcolor='b')
        
        # Add a horizontal line at 0.5 (random initialization expectation)
        ax2.axhline(y=0.5, color='gray', linestyle='--', alpha=0.7, label='Random Init (50%)')
        
        # Add vertical lines for correlation thresholds
        for threshold in correlation_thresholds:
            for i, (epoch, corr) in enumerate(self.correlation_history):
                if corr > threshold:
                    ax1.axvline(x=epoch, color=f'C{correlation_thresholds.index(threshold)+3}', 
                              linestyle='--', alpha=0.7,
    ...
    def run_analysis(self, n_epochs=5000, snapshot_interval=1000, metric_interval=100):
        ...
        # 5. NEW: Create theory validation visualization
        print("\n----- Validating Theoretical Predictions -----")
        self.visualize_theory_validation()
        
        # 6. Analyze distributed computation
        print("\n----- Analyzing Distributed Computation -----")
        computation_analysis = self.analyze_distributed_computation()
        
        # 7. Perform parity check analysis
...
def run_single_parity_analysis(d, k, epochs=10000, M1=128, M2=128, learning_rate=0.01):
    """
    ...
    # Run analysis
    analyzer.run_analysis(n_epochs=epochs, snapshot_interval=epochs//10, metric_interval=epochs//50)
...
def analyze_learning_scaling():
    """
    Run experiments with different dimensions and parity orders 
    to analyze the learning scaling behavior
    """
    # Configuration for the scaling analysis
    configs = [
        # (d, k, epochs)
        (20, 3, 5000),
        (20, 4, 5000),
    ...
    for d, k, epochs in configs:
        print(f"\n\n{'='*80}")
        print(f"Running scaling analysis for d={d}, k={k}, epochs={epochs}")
        print(f"{'='*80}\n")
        
        save_dir = f"parity_scaling_d{d}_k{k}"
        
        # Create analyzer with larger network for more capacity
        analyzer = ParityNetworkAnalyzer(
            d=d,
            k=k,
            M1=256,
        ...
        # Find correlation milestones
        corr_milestones = {}
        for threshold in [0.5, 0.9, 0.95]:
            for epoch, corr in analyzer.correlation_history:
                if corr > threshold:
                    corr_milestones[threshold] = epoch
                    break
            if threshold not in corr_milestones:
                corr_milestones[threshold] = epochs  # If never reached
        ...
        results.append({
            ...
            'time_to_corr_0.5': corr_milestones.get(0.5, epochs),
            'time_to_corr_0.9': corr_milestones.get(0.9, epochs),
            'time_to_corr_0.95': corr_milestones.get(0.95, epochs),
            'training_time': end_time - start_time
        })
        
        # Save results so far
        result_df = pd.DataFrame(results)
        result_df.to_csv("parity_scaling_results.csv", index=False)
    
    # Create visualization of the scaling results
...
def create_scaling_visualization(results):
    """
    Create visualizations of the scaling behavior
    """
    df = pd.DataFrame(results)
    
    # Convert to time to correlation 0.9 vs dimensions, grouped by k
    plt.figure(figsize=(12, 8))
    ...
    for i, k in enumerate(sorted(df['k'].unique())):
        subset = df[df['k'] == k]
        plt.plot(subset['d'], subset['time_to_corr_0.9'], 
                marker=markers[i % len(markers)], linestyle='-', linewidth=2,
                label=f'k={k}')
    
    # Add theoretical scaling curves for comparison
    dimensions = np.array(sorted(df['d'].unique()))
    
    # Add d^1 scaling (the specialization phase)
    plt.plot(dimensions, dimensions * 50, 'k--', alpha=0.7, label=r'$\Theta(d)$ scaling')
    
    # Add d^(k/2-1) scaling for each k (learning phase)
    for i, k in enumerate(sorted(df['k'].unique())):
        power = k/2 - 1
        # Scale to match approximately the observed data
        if power > 0:
            scale_factor = 10
            plt.plot(dimensions, scale_factor * dimensions**power, ':', color=f'C{i}', 
                   alpha=0.7, label=f'$\\Theta(d^{{{power:.1f}}})$ scaling (k={k})')
    
    plt.xlabel('Input Dimension (d)')
    plt.ylabel('Training Time to Correlation > 0.9 (epochs)')
    plt.title('Scaling of Training Time with Input Dimension')
    plt.grid(True)
    plt.legend()
    
    plt.tight_layout()
    plt.savefig("parity_scaling_time_vs_dimension.png", dpi=300)
    plt.close()
    
    # Create a log-log plot to better visualize the power law scaling
    plt.figure(figsize=(12, 8))
    
    for i, k in enumerate(sorted(df['k'].unique())):
        subset = df[df['k'] == k]
        plt.loglog(subset['d'], subset['time_to_corr_0.9'], 
                 marker=markers[i % len(markers)], linestyle='-', linewidth=2,
                 label=f'k={k}')
    
    # Add theoretical scaling lines
    for i, k in enumerate(sorted(df['k'].unique())):
        power = max(1, k/2 - 1)  # Use max to account for both phases
        scale_factor = 50 if power == 1 else 10
        plt.loglog(dimensions, scale_factor * dimensions**power, ':', color=f'C{i}', 
                alpha=0.7, label=f'$\\Theta(d^{{{power:.1f}}})$ theory (k={k})')
    
    plt.xlabel('Input Dimension (d) - log scale')
    plt.ylabel('Training Time (epochs) - log scale')
    plt.title('Power Law Scaling of Training Time with Input Dimension')
    plt.grid(True, which="both", ls="-")
    plt.legend()
    
    plt.tight_layout()
    plt.savefig("parity_scaling_time_vs_dimension_loglog.png", dpi=300)
    plt.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--mode', type=str, default='single', choices=['single', 'scaling'],
                      help='Run mode: single analysis or scaling study')
    ...
    if args.mode == 'single':
        run_single_parity_analysis(args.d, args.k, args.epochs, args.M1, args.M2, args.lr)
    else:
        analyze_learning_scaling()