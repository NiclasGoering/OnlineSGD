...
class EnsembleParityExperiment:
    ...
    def __init__(self, d=30, k=6, M1=512, learning_rate=0.01, max_epochs=50000,
        ...
        # --- Fixed Datasets ---
        self.X_train_eval = torch.tensor(np.random.choice([-1, 1], size=(test_set_size, d)), dtype=torch.float32).to(self.device)
        self.y_train_eval = self._target_function(self.X_train_eval)
    ...
    def _compute_loss_metrics(self, epoch):
        ...
        for model in self.models:
            model.eval()
            with torch.no_grad():
                # Calculate training loss on the fixed training evaluation set
                train_preds = model(self.X_train_eval)
                train_loss = self.criterion(train_preds, self.y_train_eval)
...
# ------------------------------------------------------------------
    ...
    def _create_optimizer(self, model):
        ...
        return torch.optim.RAdam(param_groups, lr=self.learning_rate)  #SGD
    ...
    def train(self, early_stop_corr=0.995):
        """
        Main training loop for the ensemble. Initializes diagnostics, then trains
        all models until convergence or max_epochs is reached.
        """
        print(f"Starting training for up to {self.max_epochs} epochs...")
        
        # --- FIX: Initialization must happen BEFORE the first metrics call ---
            ...
            while not all(self.is_converged) and epoch < self.max_epochs:
                ...
                for i in range(0, len(active_indices), self.n_parallel):
                    ...
                    for model_idx in chunk_indices:
                        ...
                        model.train()
                        loss = self.criterion(model(X_batch), y_batch_scaled)
    ...
    # --- MODIFIED: Plot for single-model m_A, colored by subset size ---
    def plot_walsh_order_parameters_single(self):
        ...
        Lines are colored based on the size of the subset |A|, so all subsets of the
        same size will have the same color.
        """
        print("Plotting single-model Walsh order parameter (m_A) evolution (colored by |A|)...")
        if not self.walsh_single_model_stats[0]['epochs']:
            print("Walsh order parameter data not available.")
            return

        # This import is needed for the custom legend
        ...
        fig, axs = plt.subplots(nrows, ncols, figsize=(4.5 * ncols, 4 * nrows), sharex=True, sharey=True, squeeze=False)
        axs = axs.flatten()
        
        # --- Create a color map based on subset size |A| ---
        # There are k possible sizes, from 1 to k. We get k distinct colors.
        size_colors = cm.get_cmap('rainbow', self.k) 
        ...
        for i in range(num_models):
            ...
            # Iterate through the subsets to plot each line
            for j, subset_tensor in enumerate(self.walsh_param_sets):
                label = self.walsh_param_labels[j]
                
                # Highlight the full teacher subset S
                is_teacher_subset = 'm_S' in label
                lw = 2.5 if is_teacher_subset else 1.5
                ls = '-' if is_teacher_subset else '--'
                alpha = 1.0 if is_teacher_subset else 0.8
                
                # --- Get color based on subset size ---
                subset_size = len(subset_tensor)
                # Map size (1 to k) to colormap index (0 to k-1)
                color = size_colors((subset_size - 1) / (self.k -1)) if self.k > 1 else size_colors(0.5)

                ax.plot(epochs, m_A_values[:, j], lw=lw, ls=ls, color=color, alpha=alpha)
        ...
        # --- Add shared labels and a single, custom legend for the colors ---
        fig.supxlabel('Epoch', fontsize=14)
        fig.supylabel(r'Projection Value $m_A = \langle f, \chi_A \rangle$', fontsize=14)
        fig.suptitle('Evolution of Walsh Order Parameters (Colored by Subset Size $|A|$)', fontsize=16)
        
        # Create a custom legend for the colors, as the default one is not suitable here.
        legend_elements = []
        for size in range(1, self.k + 1):
            color = size_colors((size - 1) / (self.k -1)) if self.k > 1 else size_colors(0.5)
            # Add a bold and larger style for the teacher function's size in the legend
            label = f'$|A| = {size}$'
            if size == self.k:
                legend_elements.append(Line2D([0], [0], color=color, lw=2.5, label=f'$\\bf{{|A| = {size}}}$ (Teacher)'))
            else:
                 legend_elements.append(Line2D([0], [0], color=color, lw=2, label=label))

            
        fig.legend(handles=legend_elements, loc='upper right', ncol=1, fontsize=10, title="Subset Size")
        ...
        # Save to a new file to not overwrite the old plot
        plt.savefig(f"{self.save_dir}/walsh_mA_evolution_single_models_by_size.png", dpi=300)
        plt.close()
...
if __name__ == "__main__":
    ...
    config = {
        ...
        'device_id': 0,
        'gamma': 1.0,
        'save_dir': "/home/goring/OnlineSGD/results_ana/2404_parity_relu_radam_45_10_walsh_",
        'a_init_scale': 1.0,      # <<<--- ADD THIS HYPERPARAMETER
        'test_set_size': 20000
    }
    ...
    experiment.run_and_plot_final_diagnostics() # <<<--- ADD THIS LINE
    
    #run_d_scaling_diagnostic(config)
