...
class ParityNetAnalyzer:
    ...
    def train(self, n_epochs=10000, early_stop_corr=0.9999):
        ...
        self.compute_s_m_squared(0) # ADDED: Compute initial s_m_squared values
            ...
            if epoch % self.tracker == 0 or epoch == n_epochs:
                ...
                with torch.no_grad():
                    ...
                    if torch.var(preds_squeezed) == 0 or torch.isnan(torch.var(preds_squeezed)) or len(preds_squeezed) <=1 :
                        correlation = 0.0
                        # print(f"Warning: Preds var is zero or NaN, or not enough elements. Setting correlation to 0. Var: {torch.var(preds_squeezed)}")
                    else:
                        try:
                            ...
                            # print(f"Error calculating correlation: {e}. Falling back to numpy.")
                ...
                self.compute_s_m_squared(epoch) # ADDED: Compute s_m_squared values
                ...
                if self.W1_grad and self.gradient_correlations['mean_rel_rel']: # Check if list is not empty
                    grad_corrs_rel_rel = self.gradient_correlations['mean_rel_rel'][-1]
                    grad_corrs_rel_irrel = self.gradient_correlations['mean_rel_irrel'][-1]
                    # print(f"Gradient correlations: Rel-Rel = {grad_corrs_rel_rel:.4f}, Rel-Irrel = {grad_corrs_rel_irrel:.4f}")
    ...
    def plot_all(self):
        ...
        # Activation plots (using x^2 activation)
        self.plot_activation_statistics() # For x^2, shows % non-zero output
        self.plot_gradient_correlations()
        self.plot_hidden_target_correlations() # For x^2 activation
        self.plot_gradient_eigenvalues()
        self.plot_principal_angles()
        
        # Neuron activation evolution plots (using x^2 activation)
        if self.neuron_activation_evolution['epochs']:
            print("Plotting neuron non-zero output evolution...")
            self.plot_neuron_activation_evolution()
            print("Plotting individual neuron non-zero output trajectories...")
            self.plot_neuron_activation_trajectories()
        
        # Enhanced neuron activation analysis for final state (using x^2 activation)
        print("Running neuron non-zero output analysis for final state (2^k)...")
        self.analyze_neuron_activations() # This also calls plot_neuron_activation_ratios
        ...
        if self.macroscopic_quantities['epochs']:
            ...
            print("Plotting macroscopic quantities (r_i, p_j) trajectories...")
            self.plot_macroscopic_quantities_trajectories()
            print("Plotting macroscopic quantities (r_i, p_j) distributions...")
            self.plot_macroscopic_quantities_distribution()

        # New collective field plots (s_j^2, m_a^2, phi1, phi2)
        if self.s_m_squared_evolution['epochs']:
            print("Plotting s_j^2 and m_a^2 evolution (heatmaps)...")
            self.plot_s_m_squared_evolution()
            print("Plotting phi1 and phi2 evolution (line plots)...")
            self.plot_phi_values_evolution()
            
        # Visualize second layer patterns (based on 2^k analysis for x^2 activation)
...
if __name__ == "__main__":
    ...
    k = 8           # Parity function order (k-sparse parity)
    M1 = 1024        # First layer width
    M2 = 1024        # Second layer width
    batch_size = 10000
    learning_rate = 0.0001
    n_epochs = 50000  # Reduced for testing, use larger value (e.g., 20000) for final runs
    tracker = 500     # Compute metrics every 100 epochs
    top_k_eigen = 5
    base_save_dir = "parity_analysis_experiments_456_0306_big2"
