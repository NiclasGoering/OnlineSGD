...
class ParityNetAnalyzer:
    def __init__(self, d=30, k=6, M1=512, M2=512, learning_rate=0.01,
                    ...
                    tracker=20, top_k_eigen=5, W1_grad=True, W2_grad=True, a_grad=True,
                    W1_init='random', kernel_num=100000, kernel_samples=10000,
                    # New parameters for Langevin Dynamics
                    weight_decay=0.01, temperature=0.1):
            """
            ...
            Args:
                ...
                learning_rate: Learning rate for training (η in the paper)
                weight_decay: Strength of the weight decay (γ in the paper)
                temperature: Magnitude of the injected noise (T in the paper)
            ...
            # Store new Langevin dynamics parameters
            self.weight_decay = weight_decay
            self.temperature = temperature

            self.kernel_samples = kernel_samples
            self.kernel_num = kernel_num
            self.kernel1_stats = {'epochs': [], 'kernels': [], 'eigs': []}
            self.kernel2_stats = {'epochs': [], 'kernels': [], 'eigs': []}
            ...
            # The optimizer is no longer needed, as updates are manual
            # self.optimizer = torch.optim.Adam(...)
            self.criterion = torch.nn.MSELoss()

            # --- The rest of the __init__ method remains the same ---
            # (loss_history, correlation_history, feature_correlations, etc.)
            self.loss_history = []
            self.correlation_history = []
            self.feature_correlations = {
                'epochs': [], 'relevant_features': [], 'irrelevant_features': [],
                'relevant_mean': [], 'irrelevant_mean': [], 'relevant_std': [], 'irrelevant_std': [],
            }
            self.empirical_signal_noise = {
                'epochs': [], 'empirical_signal': [], 'empirical_noise': [], 'empirical_ratio': [],
                'theoretical_signal': [], 'theoretical_noise': [], 'theoretical_ratio': []
            }
            self.gradient_stats = {
                'epochs': [], 'relevant_grad_mean': [], 'irrelevant_grad_mean': [],
                'relevant_grad_std': [], 'irrelevant_grad_std': [], 'w1_grad_norm': []
            }
            ...
            self.phase_transition = {
                'detected': False, 'epoch': None, 'correlation': None,
                'feature_ratio': None, 'theoretical_epoch': self.get_theoretical_transition_epoch()
            }
            self.activation_stats = {'epochs': [], 'layer1_percent_active': [], 'layer2_percent_active': []}
            self.gradient_correlations = {
                'epochs': [], 'rel_rel_corr': [], 'rel_irrel_corr': [], 'irrel_irrel_corr': [],
                'mean_rel_rel': [], 'mean_rel_irrel': [], 'mean_irrel_irrel': []
            }
            self.hidden_target_correlations = {'epochs': [], 'layer1_corr_sum': [], 'layer2_corr_sum': []}
            self.gradient_eigen = {
                'epochs': [], 'W1_eigenvalues': [], 'W2_eigenvalues': [],
                'W1_eigenvectors': [], 'W2_eigenvectors': []
            }
            ...
            self.neuron_activation_analysis = {'layer1_activation_ratio': None, 'layer2_activation_ratio': None}
            self.neuron_activation_evolution = {
                'epochs': [], 'layer1_activation_ratios': [], 'layer2_activation_ratios': []
            }
            self.neuron_target_correlation_evolution = {
                'epochs': [], 'layer1_neuron_correlations': [], 'layer2_neuron_correlations': []
            }
            self.second_layer_patterns = {'active_neurons': None, 'input_patterns': {}}
            self.macroscopic_quantities = {'epochs': [], 'r_values': [], 'p_values': []}
            self.s_m_squared_evolution = {
                'epochs': [], 's_squared_values': [], 'm_squared_values': [],
                'phi1': [], 'phi2': []
            }

            print(f"ParityNetAnalyzer initialized on {self.device} for Langevin Dynamics")
            print(f"Analyzing {k}-parity function in {d} dimensions with x^2 activation")
            print(f"Network: {M1} → {M2} → 1")
            print(f"Langevin Params: η={self.learning_rate}, T={self.temperature}, γ={self.weight_decay}")
    ...
    def compute_layer1_kernel(self, epoch):
        """
        Compute the layer-1 kernel
            K1 = H1ᵀ H1 / kernel_num
        and its eigenvalues. On failure, fall back to NumPy.
        """
        self.model.eval()
        # draw random inputs
        X = torch.tensor(
            np.random.choice([-1, 1], size=(self.kernel_num, self.d)),
            dtype=torch.float32
        ).to(self.device)

        with torch.no_grad():
            H1 = self.model.get_first_layer_activations(X)  # (kernel_num × M1)
            K1 = (H1.t() @ H1) / float(self.kernel_num)     # normalize by sample count
        ...
        # store
        self.kernel1_stats['epochs'].append(epoch)
        self.kernel1_stats['kernels'].append(K1.cpu().numpy())
        self.kernel1_stats['eigs'].append(eigs)


    def compute_layer2_kernel(self, epoch):
        """
        Compute the layer-2 kernel
            K2 = H2ᵀ H2 / kernel_num
        and its eigenvalues. On failure, fall back to NumPy.
        """
        self.model.eval()
        X = torch.tensor(
        ...
        with torch.no_grad():
            H2 = self.model.get_second_layer_activations(X) # (kernel_num × M2)
            K2 = (H2.t() @ H2) / float(self.kernel_num)      # normalize by sample count

        try:
            eigs = torch.linalg.eigvalsh(K2).cpu().numpy()
        except (torch.linalg.LinAlgError, RuntimeError):
            K2_cpu = K2.cpu().numpy()
            try:
                eigs = np.linalg.eigvalsh(K2_cpu)
            except Exception:
                eigs = np.zeros(K2_cpu.shape[0], dtype=float)

        self.kernel2_stats['epochs'].append(epoch)
        self.kernel2_stats['kernels'].append(K2.cpu().numpy())
        self.kernel2_stats['eigs'].append(eigs)
    ...
    def train(self, n_epochs=10000, early_stop_corr=0.9999):
        """
        Train the network using Langevin Dynamics and track relevant metrics.
        The update rule follows Eq. (1) from Naveh et al. (2021).
        """
        print(f"Starting Langevin dynamics training for {n_epochs} epochs...")
        ...
        self.compute_layer1_kernel(0)
        self.compute_layer2_kernel(0)

        start_time = time.time()

        for epoch in tqdm(range(1, n_epochs + 1), desc="Training Progress"):
            # --- Training step with Langevin Dynamics ---
            self.model.train()
            X = torch.tensor(
                np.random.choice([-1, 1], size=(self.batch_size, self.d)),
                dtype=torch.float32
            ).to(self.device)
            y = self._target_function(X)
            ...
            # Backward pass to compute gradients ∇w L(zw)
            loss.backward()

            # --- Manual Weight Update (Langevin Dynamics) ---
            with torch.no_grad():
                for p in self.model.parameters():
                    if p.requires_grad:
                        # Gradient of the loss from backpropagation
                        grad_L = p.grad
                        
                        # Weight decay term: -η * γ * w
                        weight_decay_term = self.weight_decay * p.data
                        
                        # Noise term: sqrt(2 * T * η) * ξ
                        # where ξ is a standard normal random variable
                        noise = torch.randn_like(p.data) * np.sqrt(2 * self.temperature * self.learning_rate)
                        
                        # Combine terms for the full update
                        # Δw = -η * (γ*w + ∇L) + noise
                        update = -self.learning_rate * (weight_decay_term + grad_L) + noise
                        
                        # Apply the update to the parameter
                        p.data.add_(update)

            # Record loss (no change here)
            self.loss_history.append(loss.item())

            # --- Evaluation & metrics at intervals (no change here) ---
            if epoch % self.tracker == 0 or epoch == n_epochs:
                # This entire block for evaluation remains exactly the same
                self.model.eval()
                with torch.no_grad():
                    preds = self.model(self.X_test)
                    test_loss = self.criterion(preds, self.y_test).item()
                    preds_s = preds.squeeze()
                    y_s = self.y_test.squeeze()
                ...
                if not self.phase_transition['detected'] and correlation > 0.99:
                    ...
                    # ... (rest of the evaluation block)
                    self.phase_transition['correlation'] = correlation
                    if self.feature_importance['epochs']:
                        idx = min(range(len(self.feature_importance['epochs'])),
                                  key=lambda i: abs(self.feature_importance['epochs'][i]-epoch))
                        self.phase_transition['feature_ratio'] = self.feature_importance['ratio'][idx]
                ...
                self.compute_layer1_kernel(epoch)
                self.compute_layer2_kernel(epoch)
                elapsed = time.time() - start_time
                print(f"Epoch {epoch}: MSE={test_loss:.6f}, Corr={correlation:.4f}, Time={elapsed:.1f}s")

                if correlation > early_stop_corr:
                    print(f"Early stopping at epoch {epoch} with correlation {correlation:.4f}")
                    # Final computation and return logic remains the same
                    return correlation, epoch
        
        # --- Final evaluations at the end of training ---
        # (This block also remains the same)
        self.model.eval()
        with torch.no_grad():
            preds = self.model(self.X_test)
            final_loss = self.criterion(preds, self.y_test).item()
            preds_s = preds.squeeze(); y_s = self.y_test.squeeze()
            if preds_s.numel()>1 and torch.var(preds_s)>0 and torch.var(y_s)>0:
                final_corr = torch.corrcoef(torch.stack([preds_s, y_s]))[0,1].item()
                final_corr = 0.0 if torch.isnan(torch.tensor(final_corr)) else final_corr
            else:
                final_corr = 0.0
        
        self.compute_layer1_kernel(n_epochs)
        self.compute_layer2_kernel(n_epochs)
        
        # IPR calculations also remain the same
        K2_final = self.kernel2_stats['kernels'][-1]
        try:
            eigvals, eigvecs = np.linalg.eigh(K2_final)
        except Exception:
            eigvals, eigvecs = np.linalg.eig(K2_final)
            eigvecs = eigvecs.real

        print("\n=== Inverse Participation Ratios (layer-2 top-k PCs) ===")
        for a in range(self.k):
            w = np.abs(eigvecs[:, -self.k + a])
            pr = (w**2).sum()**2 / (w**4).sum()
            print(f"PC {a}: effective # neurons ≈ {pr:.1f}")
        print("===========================================================\n")
    ...
    def plot_layer2_projection_matrix(self):
        """
        At phase transition, build P = V_topk @ V_topk^T and heatmap it.
        """
        if not self.phase_transition['detected']:
            print("No phase transition detected; skipping projection‐matrix plot.")
            return

        # find the stored K2 index at phase transition
        pt = self.phase_transition['epoch']
        idx = min(range(len(self.kernel2_stats['epochs'])),
                key=lambda i: abs(self.kernel2_stats['epochs'][i] - pt))

        # get the eigenvectors from that K2
        K2 = self.kernel2_stats['kernels'][idx]
        try:
            eigvals, eigvecs = np.linalg.eigh(K2)
        except Exception:
            eigvals, eigvecs = np.linalg.eig(K2)
            eigvecs = eigvecs.real

        # take top-k eigenvectors
        V = eigvecs[:, -self.k:]     # shape (M2 x k)
        P = V @ V.T                  # projection matrix (M2 x M2)

        # plot
        fig, ax = plt.subplots(figsize=(6,6))
        im = ax.imshow(P, vmin=0, vmax=1, cmap='viridis')
        ax.set_title(f"Layer-2 PC Projection P @ Epoch {pt}")
        ax.set_xlabel("Neuron index j")
        ax.set_ylabel("Neuron index i")
        fig.colorbar(im, ax=ax, fraction=0.046, pad=0.04)

        plt.tight_layout()
        plt.savefig(f"{self.save_dir}/layer2_projection_P_d{self.d}_k{self.k}_pt{pt}.png", dpi=300)
        plt.close()


    def plot_layer1_top20_eigen_normalized(self):
        """
        Plot the top-20 eigenvalues of the layer-1 kernel over epochs,
        each normalized by the largest eigenvalue at epoch=0,
        using the viridis colormap and a log y-axis.
        """
        stats = self.kernel1_stats
        if not stats['epochs']:
            print("No layer-1 kernel eigen data to plot.")
            return

        # Stack into shape (T × M1)
        eigs_all = np.stack(stats['eigs'], axis=0)
        epochs = stats['epochs']

        # sort descending along axis=1
        eigs_desc = np.sort(eigs_all, axis=1)[:, ::-1]   # largest-first
        top20 = eigs_desc[:, :20]                        # (T × 20)

        # normalization factor = λ_max at epoch0
        norm0 = top20[0, 0] if top20.shape[0] > 0 else 1.0
        ...
        ax.set_title(f"Layer-1 Top-20 Eigenvalues (normalized)  d={self.d}, k={self.k}")
        ax.grid(True, which='both', linestyle=':', linewidth=0.5)

        # optional legend if you want
        # ax.legend([f"#{i+1}" for i in range(20)], ncol=4, fontsize='small')

        plt.tight_layout()
        plt.savefig(f"{self.save_dir}/layer1_top20_eigen_normalized_d{self.d}_k{self.k}.png", dpi=300)
        plt.close()


    def plot_layer2_top20_eigen_normalized(self):
        """
        Plot the top-20 eigenvalues of the layer-2 kernel over epochs,
        each normalized by the largest eigenvalue at epoch=0,
        using the viridis colormap and a log y-axis.
        """
        stats = self.kernel2_stats
        if not stats['epochs']:
            print("No layer-2 kernel eigen data to plot.")
            return
        ...
        ax.set_xlabel("Epoch")
        ax.set_ylabel("Eigenvalue / λ₀₀")
        ax.set_title(f"Layer-2 Top-20 Eigenvalues (normalized)  d={self.d}, k={self.k}")
        ax.grid(True, which='both', linestyle=':', linewidth=0.5)

        plt.tight_layout()
        plt.savefig(f"{self.save_dir}/layer2_top20_eigen_normalized_d{self.d}_k{self.k}.png", dpi=300)
        plt.close()

    def plot_layer1_kernel_heatmaps(self):
        """
        Linear‐scale heatmaps of K¹ at epoch 0, at phase transition, and at final.
        """
        stats = self.kernel1_stats
        if not stats['epochs']:
            print("No layer-1 kernel data to plot.")
            return

        idxs = [0]
        if self.phase_transition['detected']:
            pt = self.phase_transition['epoch']
            idxs.append(min(range(len(stats['epochs'])),
                            key=lambda i: abs(stats['epochs'][i] - pt)))
        idxs.append(len(stats['epochs']) - 1)

        fig, axs = plt.subplots(1, len(idxs), figsize=(5 * len(idxs), 5))
        for ax, i in zip(axs, idxs):
            K = stats['kernels'][i]
            im = ax.imshow(K, aspect='auto', cmap='viridis')
            ax.set_title(f"Epoch {stats['epochs'][i]}")
            ax.set_xlabel("Neuron index j")
            ax.set_ylabel("Neuron index i")
            fig.colorbar(im, ax=ax, fraction=0.046, pad=0.04)

        plt.tight_layout()
        plt.savefig(f"{self.save_dir}/kernel1_heatmaps_linear_d{self.d}_k{self.k}.png", dpi=300)
        plt.close()

    def plot_revived_vs_eigen(self):
        """
        Sanity check at phase transition:
        • Alive neurons = var[H2] > 1e-6
        • Revived eigen neurons = max_abs loading in top-k PCs > 0.1
        Scatter each neuron by (variance, max_loading) and draw thresholds.
        """
        if not self.phase_transition['detected']:
            print("No phase transition detected; skipping revived-vs-eigen plot.")
            return

        # 1) find PT index in stored stats
        pt_epoch = self.phase_transition['epoch']
        pt_idx = min(range(len(self.kernel2_stats['epochs'])),
                     key=lambda i: abs(self.kernel2_stats['epochs'][i] - pt_epoch))

        # 2) Recompute H2 at PT on fresh samples
        self.model.eval()
        X = torch.tensor(
            np.random.choice([-1,1], size=(self.kernel_num, self.d)),
            dtype=torch.float32
        ...
        with torch.no_grad():
            H2 = self.model.get_second_layer_activations(X)  # (kernel_num x M2)
        variances = H2.var(dim=0).cpu().numpy()             # shape (M2,)

        # alive mask
        alive_mask = variances > 1e-6

        # 3) Get stored K2 and its top-k eigenvectors
        K2 = self.kernel2_stats['kernels'][pt_idx]
        # eigh ascending
        try:
            eigvals, eigvecs = np.linalg.eigh(K2)
        except Exception:
            eigvals, eigvecs = np.linalg.eig(K2)            # fallback
            eigvecs = eigvecs.real
        topk_vecs = eigvecs[:, -self.k:]                    # (M2 x k)
        # max abs loading per neuron
        max_loading = np.max(np.abs(topk_vecs), axis=1)
        revived_mask = max_loading > 0.1

        # 4) Plot scatter
        fig, ax = plt.subplots(figsize=(8,6))
        ax.scatter(variances, max_loading,
                   c=['red' if alive_mask[i] else 'gray'
                      for i in range(len(variances))],
                   s=10, alpha=0.6,
                   label="neurons")
        # thresholds
        ax.axvline(1e-6, color='black', linestyle='--', label='var thresh')
        ax.axhline(0.1, color='black', linestyle=':', label='loading thresh')

        ax.set_xscale('log')
        ax.set_yscale('log')
        ax.set_xlabel("Var[h²ᵢ]")
        ax.set_ylabel("max |loading| in top-k PCs")
        ax.set_title(f"Alive vs Revived Eigen Neurons @ Epoch {pt_epoch}")
        ax.legend(loc='upper right', fontsize='small')
        ax.grid(True, which='both', ls=':', lw=0.5)

        plt.tight_layout()
        plt.savefig(f"{self.save_dir}/revived_vs_eigen_d{self.d}_k{self.k}_pt{pt_epoch}.png", dpi=300)
        plt.close()


    def plot_layer2_kernel_heatmaps(self):
        """
        Linear‐scale heatmaps of K² at epoch 0, at phase transition, and at final.
        """
        stats = self.kernel2_stats
        if not stats['epochs']:
            print("No layer-2 kernel data to plot.")
            return
        ...
        plt.tight_layout()
        plt.savefig(f"{self.save_dir}/kernel2_heatmaps_linear_d{self.d}_k{self.k}.png", dpi=300)
        plt.close()
    ...
    def plot_kernel_eigenvalue_evolution(self):
        """
        Plot the top-20 and bottom-20 eigenvalues of both kernels
        on a log-scale y-axis over tracked epochs.
        """
        for name, stats in [
            ('layer1', self.kernel1_stats),
            ('layer2', self.kernel2_stats),
        ]:
            if not stats['epochs']:
                print(f"No {name} kernel eigen data to plot.")
                continue

            eigs = np.stack(stats['eigs'], axis=0)   # (num_times × M)
            epochs = stats['epochs']

            # sort ascending; pick bottom-20, top-20
            sorted_eigs = np.sort(eigs, axis=1)
            bot20 = sorted_eigs[:, :20]
            top20 = sorted_eigs[:, -20:]

            fig, ax = plt.subplots(figsize=(12, 6))
            # bottom eigenvalues: dashed
            for i in range(bot20.shape[1]):
                ax.plot(epochs, bot20[:, i], linestyle='--', linewidth=1, label=None)
            # top eigenvalues: solid
            for i in range(top20.shape[1]):
                ax.plot(epochs, top20[:, i], linestyle='-', linewidth=1, label=None)

            ax.set_yscale('log')   # <-- log y-axis
            ax.set_xlabel("Epoch")
            ax.set_ylabel("Eigenvalue (log scale)")
            ax.set_title(f"{name} kernel eigenvalues (top/bottom 20)")
            ax.grid(True, which='both', ls=':', lw=0.5)

            plt.tight_layout()
            plt.savefig(f"{self.save_dir}/{name}_kernel_eig_evolution_d{self.d}_k{self.k}.png", dpi=300)
            plt.close()
    ...
    def plot_all(self):
        """
        Generate all plots in one go, including:
          - training progress
          - feature importance & correlations
          - signal/noise
          - weight & gradient diagnostics
          - rank-1 condensation & second-layer alignment
          - activation & hidden-target correlations
          - neuron activation & target-correlation evolutions
          - macroscopic quantities (r_i, p_j) and their distributions
          - s^2/m^2 and phi1/phi2
          - final-state 2^k neuron activation analyses
          - full-layer kernel heatmaps & eigenvalue evolution
        """
        print("Generating all plots...")

        # 1) Training Progress & High-Level Metrics
        self.plot_training_progress()
        self.plot_feature_importance_evolution()
        self.plot_feature_correlations()
        self.plot_signal_noise_ratio()
        self.plot_revived_vs_eigen()
        self.plot_layer2_projection_matrix()


        # 2) Weight & Gradient Analysis
        ...
        self.plot_principal_angles()
                # 12) Top-20 normalized eigenvalue trajectories
        print("Plotting normalized top-20 eigenvalue trajectories...")
        self.plot_layer1_top20_eigen_normalized()
        self.plot_layer2_top20_eigen_normalized()


        # 3) Corrected Condensation & Alignment
        print("Plotting corrected condensation and alignment diagnostics...")
        

        # 4) Activation & Hidden-Target Correlations
        self.plot_activation_statistics()
        self.plot_hidden_target_correlations()

        # 5) Neuron Activation Evolution
        ...
        # 6) Neuron-Target Correlation Evolution
        ...
        # 7) Macroscopic Quantities (r_i, p_j) and Distributions
        ...
        # 8) s^2 / m^2 and phi1, phi2
        if self.s_m_squared_evolution['epochs']:
            print("Plotting collective fields (s_j^2, m_a^2, phi1, phi2) evolution...")
            self.plot_s_m_squared_evolution()
            self.plot_phi_values_evolution()

        # 9) Final-state Neuron Activation Analysis (2^k)
        print("Running and plotting final-state neuron activation analysis (2^k)...")
        self.analyze_neuron_activations()

        # 10) Visualization of Second-Layer Patterns
        print("Visualizing final-state second layer patterns...")
        self.visualize_second_layer_patterns()

        # 11) **New**: Full-Layer Kernel Diagnostics
        print("Plotting full-layer kernel heatmaps & eigenvalue evolution...")
        self.plot_layer1_kernel_heatmaps()
        self.plot_layer2_kernel_heatmaps()
    ...
    def save_results(self):
        ...
        if self.feature_correlations['epochs']:
            # Need to handle list of lists for relevant/irrelevant_features if saving them directly
        ...
        if self.gradient_correlations['epochs']:
            # Filter out the full correlation matrices before saving summary
        ...
        # Save s_m_squared_evolution data
        if self.s_m_squared_evolution['epochs']:
            phi_df = pd.DataFrame({
...
def run_parity_experiment(d=30, k=6, M1=512, M2=512, learning_rate=0.01,
                          batch_size=512, n_epochs=10000, device_id=None,
                          save_dir="parity_analysis", tracker=20, top_k_eigen=5,
                          W1_grad=True, W2_grad=True, a_grad=True, W1_init='random',
                          # Add Langevin parameters to the function signature
                          weight_decay=0.01, temperature=0.1):
    """
    ...
    run_name_parts = [
        ...
        f"wd{weight_decay}", f"T{temperature}", # Add new params to run name
        f"W1{W1_grad}", f"W2{W2_grad}", f"a{a_grad}", f"init{W1_init}", "x2act_langevin"
    ]
    ...
    analyzer = ParityNetAnalyzer(
        ...
        W1_grad=W1_grad, W2_grad=W2_grad, a_grad=a_grad, W1_init=W1_init,
        # Pass the Langevin parameters to the analyzer
...
if __name__ == "__main__":
    ...
    top_k_eigen = 5
    kernel_num = 1000000
    base_save_dir = "parity_analysis_langevin_0706_3003_T0.001"

    # --- Parameters for Langevin Dynamics ---
    # learning_rate (η), weight_decay (γ), and temperature (T) from the paper
    ...
    print(f"n_epochs={n_epochs}, tracker={tracker}")
    print(f"Langevin Params: η={learning_rate}, γ={weight_decay}, T={temperature}")
    ...
    # Run a single experiment with the new Langevin training protocol
    print("\nRunning Experiment with Langevin Dynamics")
    analyzer1 = run_parity_experiment(
        d=d, k=k, M1=M1, M2=M2, batch_size=batch_size,
        n_epochs=n_epochs, device_id=device_id, save_dir=f"{base_save_dir}/exp_langevin",
        tracker=tracker, top_k_eigen=top_k_eigen,
        W1_grad=True, W2_grad=True, a_grad=True, W1_init='random',
        # Pass the Langevin parameters to the experiment runner
        learning_rate=learning_rate,
        weight_decay=weight_decay,
        temperature=temperature
    )
    print("Langevin dynamics experiment completed.")
    print("=" * 80)
