...
class ParityNetAnalyzer:
    def __init__(self, d=30, k=6, M1=512, M2=512, learning_rate=0.01, 
                 ...
                 tracker=20, top_k_eigen=5, W1_grad=True, W2_grad=True, a_grad=True, W1_init='random', kernel_num=100000, kernel_samples=10000):
        """
        ...
        self.optimizer = torch.optim.SGD(trainable_params, lr=learning_rate) #Adam
        ...
        # Activation tracking (using x^2 activation)
        self.activation_stats = { # Note: "active" means > 0 for x^2 activation
            'epochs': [],
        ...
        # Fixed sample for computing correlations and eta
        ...
        # Macroscopic quantities tracking (r_i, p_j)
        ...
        # Collective fields s_j^2 and m_a^2 and their means phi1, phi2
        self.s_m_squared_evolution = {
            'epochs': [],
            's_squared_values': [],  # List of arrays (M1 values per epoch) for s_j^2
            'm_squared_values': [],  # List of arrays (M2 values per epoch) for m_a^2
            'phi1': [],              # Mean of s_squared_values at each epoch
            'phi2': []               # Mean of m_squared_values at each epoch
        }
    ...
    def train(self, n_epochs=10000, early_stop_corr=0.995):
        ...
        now including full-layer kernels at layer 1 and 2.
        """
        ...
        # NEW: compute full-layer kernels at epoch 0
        ...
        for epoch in tqdm(range(1, n_epochs + 1), desc="Training Progress"):
            ...
            if epoch % self.tracker == 0 or epoch == n_epochs:
                ...
                # NEW: compute full-layer kernels
        ...
        # --- compute final kernels one last time ---
        self.compute_layer1_kernel(n_epochs)
        self.compute_layer2_kernel(n_epochs)

        # --- NEW: print Inverse Participation Ratios for top-k PCs of layer-2 ---
        #  fetch the final K2 from stats
        K2_final = self.kernel2_stats['kernels'][-1]          # numpy array (M2 x M2)
        try:
            eigvals, eigvecs = np.linalg.eigh(K2_final)       # ascending
        except Exception:
...
if __name__ == "__main__":
    ...
    d = 40          # Input dimension
    k = 5           # Parity function order (k-sparse parity)
    M1 = 1024        # First layer width
    M2 = 1024        # Second layer width
    batch_size = 2**17
    learning_rate = 0.005
    n_epochs = 2000000  # Reduced for testing, use larger value (e.g., 20000) for final runs
    tracker = 200     # Compute metrics every 100 epochs
    top_k_eigen = 5
    kernel_num = 1000000 
    base_save_dir = "parity_0806_sgd_b2^17_lr0.005_2"
    # this is sgd !!!!!!!!!!!!!!!
    ...
    print(f"Using device: {'cuda:'+str(device_id) if device_id is not None and torch.cuda.is_available() else 'cpu'}") # check cuda availability
