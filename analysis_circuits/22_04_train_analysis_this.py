...
class ParityNetAnalyzer:
    def __init__(self, d=30, k=6, M1=512, M2=512, learning_rate=0.01, 
        ...
        Args:
            ...
            a_grad: Whether readout layer is trainable (if False, a is frozen)
        ...
        e = torch.zeros(self.d, device=self.device)
        e[:self.k] = 1.0 / np.sqrt(self.k)
        self.teacher_dir = e

        # Initialize the dictionary for storing φ₁, φ₂ over epochs
        self.collective_fields = {
            'epochs': [],
            'phi1': [],
            'phi2': []
        }
        ...
        # Macroscopic quantities tracking
        self.macroscopic_quantities = {
            'epochs': [],
            'r_values': [],  # r_i values for each neuron in the first layer
            'p_values': [],  # p_j values for each neuron in the second layer
        }
    ...
    def compute_macroscopic_quantities(self, epoch, num_samples=5000):
        ...
        r_values = np.mean(W1[:, :self.k], axis=1)
        
        # Generate random samples for estimating p_j
        ...
        mean_activations = np.mean(h1, axis=0)  # E_x[σ(u_i(x;t))]
        p_values = np.zeros(self.M2)
        for j in range(self.M2):
            p_values[j] = np.mean(W2[j, :] * mean_activations)
    ...
    def compute_collective_fields(self, epoch, num_samples=None):
        """
        Compute the two collective fields:
        φ1 = (1/M1) ∑_{j=1}^{M1} (s_j)^2,   where s_j = W1[j] ⋅ e
        φ2 = (1/M2) ∑_{a=1}^{M2} (m_a)^2,   where m_a = W2[a] ⋅ η
        
        Here:
        - e = self.teacher_dir (unit‐vector on the first k features).
        - η = (1/n) ∑_μ [ y_μ · h1(x_μ) / ||h1(x_μ)|| ], computed on self.X_analysis/self.y_analysis.
        - num_samples (if given) will subsample from X_analysis to estimate η (else use all).
        """
        # Ensure model is in eval mode
        self.model.eval()
        
        # 1) Compute s_j = W1[j] ⋅ e  for each first‐layer neuron j
        #    W1 is shape (M1, d)
        with torch.no_grad():
            W1 = self.model.W1.detach()  # (M1 × d)
            # teacher_dir is length‐d
            # s = W1 @ e   → shape (M1,)
            s = torch.mv(W1, self.teacher_dir)  # (M1,)
            # φ1 = (1/M1) ∑ s_j^2
            phi1 = torch.mean(s.pow(2)).item()
        
        # 2) Compute η ∈ ℝ^{M1} via X_analysis / y_analysis
        #    Optionally subsample if num_samples is passed
        X = self.X_analysis
        y = self.y_analysis
        ...
        with torch.no_grad():
            # First‐layer preactivations → h1 (shape: (batch, M1))
            h1_all = self.model.get_first_layer_activations(X_sub)  # (n × M1)
            # Compute norms of each h1 vector: (n,)
            norms = h1_all.norm(dim=1).clamp(min=1e-8)  # avoid zero‐division
            # Normalize each row of h1_all by its norm
            h1_normed = h1_all / norms.unsqueeze(1)  # (n × M1)
            # Multiply by y_sub (shape (n,)) → broadcast to (n × M1)
            weighted = (y_sub.unsqueeze(1) * h1_normed)  # (n × M1)
            # Average over all n: η has shape (M1,)
            eta = weighted.mean(dim=0)  # (M1,)
        
        # 3) Compute m_a = W2[a] ⋅ η  for each second‐layer neuron a
        #    W2 is shape (M2, M1)
        with torch.no_grad():
            W2 = self.model.W2.detach()  # (M2 × M1)
            # m = W2 @ η  → shape (M2,)
            m = torch.mv(W2, eta)  # (M2,)
            # φ2 = (1/M2) ∑ m_a^2
            phi2 = torch.mean(m.pow(2)).item()
        
        # 4) Store into history
        self.collective_fields['epochs'].append(epoch)
        self.collective_fields['phi1'].append(phi1)
        self.collective_fields['phi2'].append(phi2)
    ...
    def train(self, n_epochs=10000, early_stop_corr=0.9999):
        ...
        self.compute_macroscopic_quantities(0)  # Track macroscopic quantities
        self.compute_collective_fields(0)       # ← NEW
        ...
        for epoch in tqdm(range(1, n_epochs + 1)):
            ...
            if epoch % self.tracker == 0 or epoch == n_epochs:
                ...
                self.compute_macroscopic_quantities(epoch)  # Track macroscopic quantities
                self.compute_collective_fields(epoch)       # ← NEW
                ...
                act_layer1 = self.activation_stats['layer1_percent_active'][-1]
                act_layer2 = self.activation_stats['layer2_percent_active'][-1]
                print(f"Active neurons: Layer 1 = {act_layer1:.2f}%, Layer 2 = {act_layer2:.2f}%")
                
                if self.W1_grad:
                    grad_corrs = (
                        self.gradient_correlations['mean_rel_rel'][-1], 
                        self.gradient_correlations['mean_rel_irrel'][-1]
                    )
    ...
    def plot_macroscopic_quantities_evolution(self):
        ...
        r_values = self.macroscopic_quantities['r_values']
        p_values = self.macroscopic_quantities['p_values']
        ...
        max_abs_r = max(np.max(np.abs(np.array(r_values))), 0.001)
        max_abs_p = max(np.max(np.abs(np.array(p_values))), 0.001)
        
        # Create image arrays
        r_data = np.array(r_values)
        p_data = np.array(p_values)
        ...
        ax1.set_xlabel('Neuron Index in Layer 1')
        ...
        fig.colorbar(im1, ax=ax1, label='r_i = (1/k)∑_{l=1}^k W^{(1)}_{il}(t)')
        ...
        ax2.set_xlabel('Neuron Index in Layer 2')
        ...
        fig.colorbar(im2, ax=ax2, label='p_j = (1/M1)∑_{i=1}^{M1} W^{(2)}_{ji}·E_x[σ(u_i(x))]')
    ...
    def plot_collective_fields_heatmap(self):
        """
        Plot φ1 and φ2 as a two‐row heatmap over epochs.
        Row 0 = φ1 evolution, Row 1 = φ2 evolution.
        """
        if not self.collective_fields['epochs']:
            print("No collective‐field data available")
            return
        
        epochs = np.array(self.collective_fields['epochs'])
        phi1_vals = np.array(self.collective_fields['phi1'])  # shape: (T,)
        phi2_vals = np.array(self.collective_fields['phi2'])  # shape: (T,)
        
        # Stack into a 2×T array: row 0 = φ1, row 1 = φ2
        data = np.vstack([phi1_vals, phi2_vals])
        ...
        # imshow with extent to label x‐axis in “epoch” units
        im = ax.imshow(
            data,
            aspect='auto',
            cmap=cmap,
            vmin=vmin,
            vmax=vmax,
            extent=[epochs[0], epochs[-1], 0, 2]  # y from 0..2 (two rows)
        )
        
        # Set y‐ticks at the middle of each row
        ax.set_yticks([0.5, 1.5])
        ax.set_yticklabels([r'$\phi_1$', r'$\phi_2$'])
        ax.set_xlabel('Epoch')
        ax.set_title(f'Collective Fields φ₁ and φ₂ Evolution (d={self.d}, k={self.k})')
        
        # Draw horizontal line separating the two rows
        ax.axhline(y=1.0, color='white', linewidth=2)
        
        # Optionally mark phase transition if detected
        if self.phase_transition['detected']:
            pt = self.phase_transition['epoch']
            ax.axvline(x=pt, color='red', linestyle='--', alpha=0.7,
                       label=f'Phase Transition (Epoch {pt})')
        ...
        plt.tight_layout()
        plt.savefig(f"{self.save_dir}/collective_fields_heatmap_d{self.d}_k{self.k}.png", dpi=300)
        plt.close()

    
    def plot_macroscopic_quantities_trajectories(self):
        """
        Plot trajectories of macroscopic quantities r_i and p_j for individual neurons over epochs.
        """
        if not self.macroscopic_quantities['epochs']:
            print("No macroscopic quantities data available")
            return
        ...
        # Extract data
        epochs = self.macroscopic_quantities['epochs']
        r_values = np.array(self.macroscopic_quantities['r_values'])
        p_values = np.array(self.macroscopic_quantities['p_values'])
        
        # Create figures for each quantity
        # Layer 1 - r_i trajectories
        fig1, ax1 = plt.subplots(figsize=(15, 8))
        
        # Find neurons with large absolute r_i
        max_abs_r = np.max(np.abs(r_values), axis=0)
        significant_r_neurons = np.argsort(max_abs_r)[-20:]  # Top 20 neurons
        
        # Prepare colormap for neuron indices
        cmap_r = plt.cm.viridis
        
        # Plot each significant neuron's r_i trajectory
        for idx, i in enumerate(significant_r_neurons):
            # Get r_i trajectory for this neuron across all epochs
            r_trajectory = r_values[:, i]
            
            # Use color based on maximum absolute value
            color = cmap_r(max_abs_r[i] / np.max(max_abs_r))
            
            # Plot with appropriate styling
            ax1.plot(epochs, r_trajectory, '-', color=color, linewidth=2, 
                    label=f'Neuron {i}' if idx < 10 else None)  # Only show first 10 in legend
        ...
        ax1.set_ylabel('r_i = (1/k)∑_{l=1}^k W^{(1)}_{il}(t)')
        ...
        plt.tight_layout()
        plt.savefig(f"{self.save_dir}/r_trajectories_d{self.d}_k{self.k}.png", dpi=300)
        plt.close(fig1)
        
        # Layer 2 - p_j trajectories
        fig2, ax2 = plt.subplots(figsize=(15, 8))
        
        # Find neurons with large absolute p_j
        max_abs_p = np.max(np.abs(p_values), axis=0)
        significant_p_neurons = np.argsort(max_abs_p)[-20:]  # Top 20 neurons
        
        # Prepare colormap for neuron indices
        cmap_p = plt.cm.viridis
        
        # Plot each significant neuron's p_j trajectory
        for idx, j in enumerate(significant_p_neurons):
            # Get p_j trajectory for this neuron across all epochs
            p_trajectory = p_values[:, j]
            
            # Use color based on maximum absolute value
            color = cmap_p(max_abs_p[j] / np.max(max_abs_p))
            
            # Plot with appropriate styling
            ax2.plot(epochs, p_trajectory, '-', color=color, linewidth=2, 
                    label=f'Neuron {j}' if idx < 10 else None)  # Only show first 10 in legend
        ...
        ax2.set_ylabel('p_j = (1/M1)∑_{i=1}^{M1} W^{(2)}_{ji}·E_x[σ(u_i(x))]')
        ...
        plt.tight_layout()
        plt.savefig(f"{self.save_dir}/p_trajectories_d{self.d}_k{self.k}.png", dpi=300)
        plt.close(fig2)
        
        # Also plot average magnitude of r_i and p_j over time
        fig3, ax3 = plt.subplots(figsize=(15, 8))
        
        # Calculate mean absolute values
        mean_abs_r = np.mean(np.abs(r_values), axis=1)
        mean_abs_p = np.mean(np.abs(p_values), axis=1)
        ...
        # Add phase transition line if detected
        if self.phase_transition['detected']:
            ax3.axvline(x=self.phase_transition['epoch'], color='red', linestyle='--', alpha=0.7,
                      label=f'Phase Transition (Epoch {self.phase_transition["epoch"]})')
        ...
        plt.tight_layout()
        plt.savefig(f"{self.save_dir}/mean_macroscopic_quantities_d{self.d}_k{self.k}.png", dpi=300)
        plt.close(fig3)
    ...
    def plot_weight_matrices(self):
        ...
        # Create figure for readout weights a
        plt.figure(figsize=(16, 5 * len(epochs_to_show)))
        ...
        for i, idx in enumerate(epochs_to_show):
            ...
            plt.title(f"{titles[i]} (Epoch {epoch}) - Readout Layer Weights (a)")
            plt.xlabel("Layer 2 Neuron")
            plt.ylabel("Weight Value")
            
            # Add vertical line at observed transition if available
            if self.phase_transition['detected']:
                detected_epoch = self.phase_transition['epoch']
                plt.axvline(x=self.M2/2, color='red', linestyle='--', alpha=0.7,
                          label=f'Detected Transition (Epoch {detected_epoch})')
            
            plt.legend()
        
        plt.tight_layout()
        plt.savefig(f"{self.save_dir}/a_weights_d{self.d}_k{self.k}.png", dpi=300)
        plt.close()
    ...
    def plot_macroscopic_quantities_distribution(self):
        """
        Plot the distribution of r_i and p_j values at different stages of training
        similar to the neuron target correlation distributions.
        """
        if not self.macroscopic_quantities['epochs']:
            print("No macroscopic quantities data available")
            return
        ...
        # Create figure with subplots - one row per macroscopic quantity, one column per selected epoch
        fig, axs = plt.subplots(2, len(selected_indices), figsize=(15, 10))
        ...
        # For each macroscopic quantity and each selected epoch, plot histogram
        for i, (values, quantity_name) in enumerate([(r_values, "r_i (First Layer)"), (p_values, "p_j (Second Layer)")]):
            for j, (idx, label) in enumerate(zip(selected_indices, epoch_labels)):
                ax = axs[i, j]
                
                # Get values for this epoch
                vals = values[idx]
                
                # Plot histogram
                ax.hist(vals, bins=30, alpha=0.7, color='skyblue' if i==0 else 'lightgreen')
                ax.axvline(x=0, color='red', linestyle='--', alpha=0.7)
                
                # Compute statistics
                mean_val = np.mean(vals)
                mean_abs_val = np.mean(np.abs(vals))
                ...
                ax.set_title(f"{quantity_name}: {label}")
                if i == 1:  # Only add x-label for bottom row
                    ax.set_xlabel("Value")
                if j == 0:  # Only add y-label for leftmost column
                    ax.set_ylabel("Number of Neurons")
        ...
        plt.tight_layout()
        plt.savefig(f"{self.save_dir}/macroscopic_quantities_distribution_d{self.d}_k{self.k}.png", dpi=300)
        plt.close()
    ...
    def plot_all(self):
        ...
        # Macroscopic quantities plots
        if self.macroscopic_quantities['epochs']:
            print("Plotting macroscopic quantities evolution...")
            self.plot_macroscopic_quantities_evolution()
            print("Plotting macroscopic quantities trajectories...")
            self.plot_macroscopic_quantities_trajectories()
            print("Plotting macroscopic quantities distributions...")
            self.plot_macroscopic_quantities_distribution()
        
        # Visualize second layer patterns
        self.visualize_second_layer_patterns()
        
        # Plot the new collective‐fields heatmap
        print("Plotting collective fields φ₁, φ₂ …")
        self.plot_collective_fields_heatmap()
    ...
    def save_results(self):
        ...
        # Save macroscopic quantities evolution if available
        if self.macroscopic_quantities['epochs']:
            # Save r_i evolution data - one row per epoch, one column per neuron
            r_evolution = np.array(self.macroscopic_quantities['r_values'])
            r_evo_df = pd.DataFrame(
                r_evolution,
                index=self.macroscopic_quantities['epochs'],
                columns=[f"r_{i}" for i in range(self.M1)]
            )
            r_evo_df.index.name = 'epoch'
            r_evo_df.to_csv(os.path.join(results_dir, "r_values_evolution.csv"))
            
            # Save p_j evolution data - one row per epoch, one column per neuron
            p_evolution = np.array(self.macroscopic_quantities['p_values'])
            p_evo_df = pd.DataFrame(
                p_evolution,
                index=self.macroscopic_quantities['epochs'],
                columns=[f"p_{j}" for j in range(self.M2)]
            )
            p_evo_df.index.name = 'epoch'
            p_evo_df.to_csv(os.path.join(results_dir, "p_values_evolution.csv"))
            
            # Save summary statistics
            macro_summary_df = pd.DataFrame({
                'epoch': self.macroscopic_quantities['epochs'],
                'mean_abs_r': [np.mean(np.abs(r)) for r in self.macroscopic_quantities['r_values']],
                'mean_abs_p': [np.mean(np.abs(p)) for p in self.macroscopic_quantities['p_values']],
                'max_abs_r': [np.max(np.abs(r)) for r in self.macroscopic_quantities['r_values']],
                'max_abs_p': [np.max(np.abs(p)) for p in self.macroscopic_quantities['p_values']]
            })
            macro_summary_df.to_csv(os.path.join(results_dir, "macroscopic_quantities_summary.csv"), index=False)
...
def run_parity_experiment(d=30, k=6, M1=512, M2=512, learning_rate=0.01, batch_size=512, 
    ...
    run_dir = f"{save_dir}_d{d}_k{k}_bs{batch_size}_lr{learning_rate}_W1{W1_grad}_W2{W2_grad}_a{a_grad}_init{W1_init}"
...
def run_macroscopic_quantities_comparison(d=30, k=6, M1=512, M2=512, learning_rates=[0.01, 0.001], 
                                         ...
                                         a_grad=True):
    """
    Run a comparison of macroscopic quantities r_i and p_j across different learning rates
    to investigate their relationship with the phase transition.
    ...
    Returns:
        results: Dictionary of analyzers for each configuration
    """
    os.makedirs(save_dir, exist_ok=True)
    
    # Create results directory
    comparison_dir = f"{save_dir}/d{d}_k{k}_comparison_a{a_grad}"
    ...
    for lr in learning_rates:
        for bs in batch_sizes:
            ...
            run_dir = f"{comparison_dir}/{run_name}"
            
            print(f"\n{'='*80}")
            print(f"Running experiment with learning_rate={lr}, batch_size={bs}, readout trainable={a_grad}")
            print(f"{'='*80}\n")
            ...
            # Store analyzer in results dictionary
...
def create_comparative_macroscopic_plots(results, save_dir, d, k):
    """
    Create comparative plots of macroscopic quantities across different runs.
    
    Args:
        results: Dictionary of analyzers for each configuration
        save_dir: Directory to save comparison plots
        d: Input dimension (for plot titles)
        k: Parity function order (for plot titles)
    """
    # Create a directory for comparative plots
    plots_dir = f"{save_dir}/comparative_plots"
    os.makedirs(plots_dir, exist_ok=True)
    
    # Plot mean absolute r values across runs
    plt.figure(figsize=(15, 8))
    
    for run_name, analyzer in results.items():
        if not analyzer.macroscopic_quantities['epochs']:
            continue
            
        epochs = analyzer.macroscopic_quantities['epochs']
        r_values = analyzer.macroscopic_quantities['r_values']
        mean_abs_r = [np.mean(np.abs(r)) for r in r_values]
        
        plt.plot(epochs, mean_abs_r, '-', linewidth=2, label=f'{run_name} - Mean |r_i|')
        
        # Add phase transition marker if detected
        if analyzer.phase_transition['detected']:
            pt_epoch = analyzer.phase_transition['epoch']
            # Find closest epoch in data
            closest_idx = min(range(len(epochs)), key=lambda i: abs(epochs[i] - pt_epoch))
            plt.scatter([pt_epoch], [mean_abs_r[closest_idx]], marker='*', s=150, 
                      label=f'{run_name} - Phase Transition (Epoch {pt_epoch})')
    
    plt.xlabel('Epoch')
    plt.ylabel('Mean Absolute r_i Value')
    plt.title(f'Comparison of Mean |r_i| Values Across Runs (d={d}, k={k})')
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"{plots_dir}/comparative_mean_r_values.png", dpi=300)
    plt.close()
    
    # Plot mean absolute p values across runs
    plt.figure(figsize=(15, 8))
    ...
    for run_name, analyzer in results.items():
        ...
        epochs = analyzer.macroscopic_quantities['epochs']
        p_values = analyzer.macroscopic_quantities['p_values']
        mean_abs_p = [np.mean(np.abs(p)) for p in p_values]
        ...
        if analyzer.phase_transition['detected']:
            ...
            # Find closest epoch in data
            closest_idx = min(range(len(epochs)), key=lambda i: abs(epochs[i] - pt_epoch))
            plt.scatter([pt_epoch], [mean_abs_p[closest_idx]], marker='*', s=150, 
                      label=f'{run_name} - Phase Transition (Epoch {pt_epoch})')
    
    plt.xlabel('Epoch')
    plt.ylabel('Mean Absolute p_j Value')
    plt.title(f'Comparison of Mean |p_j| Values Across Runs (d={d}, k={k})')
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"{plots_dir}/comparative_mean_p_values.png", dpi=300)
    plt.close()
    
    # Create a summary plot with phase transitions and theoretical predictions
    plt.figure(figsize=(12, 6))
    
    # Track x-positions for bar charts
    x_pos = np.arange(len(results))
    detected_pts = []
    theoretical_pts = []
    run_labels = []
    
    for i, (run_name, analyzer) in enumerate(results.items()):
        run_labels.append(run_name)
        
        if analyzer.phase_transition['detected']:
            detected_pts.append(analyzer.phase_transition['epoch'])
        else:
            detected_pts.append(0)
    ...
    plt.ylabel('Epoch')
    plt.title(f'Phase Transition Epochs Across Runs (d={d}, k={k})')
    plt.xticks(x_pos, run_labels, rotation=45)
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"{plots_dir}/phase_transition_comparison.png", dpi=300)
    plt.close()
    ...
    for run_name, analyzer in results.items():
        ...
        detected = analyzer.phase_transition['detected']
        detected_epoch = analyzer.phase_transition['epoch'] if detected else "Not detected"
        theoretical_epoch = analyzer.phase_transition['theoretical_epoch']
        
        # Get final correlation
        final_corr = analyzer.correlation_history[-1][1] if analyzer.correlation_history else "N/A"
        
        # Get macroscopic quantities at transition
        if detected and analyzer.macroscopic_quantities['epochs']:
            # Find index closest to transition epoch
            epochs = analyzer.macroscopic_quantities['epochs']
            closest_idx = min(range(len(epochs)), key=lambda i: abs(epochs[i] - analyzer.phase_transition['epoch']))
            r_values = analyzer.macroscopic_quantities['r_values'][closest_idx]
            p_values = analyzer.macroscopic_quantities['p_values'][closest_idx]
            mean_abs_r_at_pt = np.mean(np.abs(r_values))
            mean_abs_p_at_pt = np.mean(np.abs(p_values))
        else:
            mean_abs_r_at_pt = "N/A"
            mean_abs_p_at_pt = "N/A"
        
        # Add to summary data
        summary_data.append({
            'run_name': run_name,
            'detected_epoch': detected_epoch,
            'theoretical_epoch': theoretical_epoch,
            'final_correlation': final_corr,
            'mean_abs_r_at_pt': mean_abs_r_at_pt,
            'mean_abs_p_at_pt': mean_abs_p_at_pt
        })
    
    # Create DataFrame and save to CSV
    summary_df = pd.DataFrame(summary_data)
    summary_df.to_csv(f"{plots_dir}/run_summary.csv", index=False)
    
    print(f"Comparative plots saved to {plots_dir}")
...
if __name__ == "__main__":
    ...
    batch_size = 512
    learning_rate = 0.01
    n_epochs = 9000  # Reduced for testing, use larger value (e.g., 20000) for final runs
    tracker = 100     # Compute metrics every 100 epochs
    top_k_eigen = 5
    base_save_dir = "parity_analysis_experiments_456_rp_x2"
    ...
    # Experiment 1: Default - All layers trainable, random initialization
    print("\nRunning Experiment 1: Default - All layers trainable, random initialization")
    ...
    # # Experiment 6: Frozen readout layer a - both W1 and W2 trainable, a frozen
    # print("\nRunning Experiment 6: Frozen readout layer - Both W1 and W2 trainable, readout layer frozen")
    # analyzer6 = run_parity_experiment(
    #     d=d,
    #     k=k,
    #     M1=M1,
    #     M2=M2,
    ...
    #     n_epochs=n_epochs,
    #     device_id=device_id,
    #     save_dir=f"{base_save_dir}/exp6_frozen_a",
    #     tracker=tracker,
    #     top_k_eigen=top_k_eigen,
    #     W1_grad=True,
    #     W2_grad=True,
    #     a_grad=False,  # Readout layer is frozen
    #     W1_init='random'
    # )
    # print("Experiment 6 completed.")
    # print("=" * 80)
    
    # # Experiment 7: Only readout layer trainable
    # print("\nRunning Experiment 7: Only readout layer trainable - W1 and W2 frozen")
    # analyzer7 = run_parity_experiment(
    #     d=d,
    #     k=k,
    #     M1=M1,
    #     M2=M2,
    ...
    #     n_epochs=n_epochs,
    #     device_id=device_id,
    #     save_dir=f"{base_save_dir}/exp7_only_a_trainable",
    #     tracker=tracker,
    #     top_k_eigen=top_k_eigen,
    #     W1_grad=False,  # W1 is frozen
    #     W2_grad=False,  # W2 is frozen 
    #     a_grad=True,    # Only readout layer is trainable
    #     W1_init='random'
    # )
    # print("Experiment 7 completed.")
    # print("=" * 80)
    
    # # Experiment 8: All layers frozen - verification that no learning happens
    # print("\nRunning Experiment 8: All layers frozen - verification that no learning happens")
    # analyzer8 = run_parity_experiment(
    #     d=d,
    #     k=k,
    #     M1=M1,
    #     M2=M2,
    #     batch_size=batch_size,
    #     learning_rate=learning_rate,
    #     n_epochs=n_epochs // 10,  # Reduced epochs since no learning happens
    #     device_id=device_id,
    #     save_dir=f"{base_save_dir}/exp8_all_frozen",
    #     tracker=tracker,
    #     top_k_eigen=top_k_eigen,
    #     W1_grad=False,  # W1 is frozen
    #     W2_grad=False,  # W2 is frozen 
